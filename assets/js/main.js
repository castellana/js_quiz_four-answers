//Quiz. Verwende das Array im Kommentarbereich und die DOM-Eigenschaften, um ein Quiz zu erstellen.
...
for (let i = 0; i < data.length; i++) {
  // console.log(data[i]);
    ...
    for(let j = 0; j < data[i].choice.length; j++){
        ...
        // btn.addEventListener('click', () => console.log("test", i, j));
        btn.addEventListener('click', () => {  //**desde aquí versión 2 */
          console.log(data[i].choice[j]);
          console.log(data[i].answer); //wolen vergleichen ob die choice die gerade geklickt wurde, die gleiche wir ??ß ist
...
// '**Versión 2, version cuando hacemos click en un button, todos se colorean

// btn.addEventListener('click', (event) => {
// if (data[i].choice[j] === data[i].answer) {
//   console.log(event);
//   console.log(event.target.parentNode.childNodes);
//   for (let k= 0; k< event.target.parentNode.childNodes.length; k++) {
//     if (event.target.parentNode.childNodes[k].tagName === 'BUTTON') {
//       console.log(event.target.parentNode.childNodes[k]);

//       if  (event.target.parentNode.childNodes[k].innerHTML == data[i].answer.toString()) {
        
//           console.log('richtig');
//           event.target.parentNode.childNodes[k].style.background = 'lime'
    
//       } else {
//           console.log('falsch');
//           event.target.parentNode.childNodes[k].style.background = 'tomato'
//       } 
//       event.target.parentNOde.childNodes[k].disabled = true
//       }
//   }
// })
